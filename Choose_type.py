...
def make_choice():
    while True:
        ...
        print("Выберите тип термоапары:")

        print("1.   Платиновые ТС и ЧЭ, α = 0,00385 °С-1 ")

        print("2.	 Платиновые ТС и ЧЭ, α = 0,00391 °С-1")

        print("3.   Медные ТС и ЧЭ, α = 0,00428 °С-1")

        print("4.   Никелевые ТС и ЧЭ, α = 0,00617 °С-1")

        number_of_termo = int(input())
        ...
        if number_of_termo == 1:
            ...
            if -60 <= temperature < 100:
                ...
                result = zero_resist * (1 + a*temperature+b*math.pow(temperature,2)+c*math.pow(temperature,2)*(temperature-100))
